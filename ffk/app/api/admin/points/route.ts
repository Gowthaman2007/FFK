...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (
      !tournamentId ||
      !Number.isFinite(killValue) ||
      killValue < 0
    ) {
      return NextResponse.json(
        { ok: false, error: "Invalid point settings." },
        { status: 400 }
      );
    }
    ...
    if (rows.length > 0) {
      const { error } = await supabase
        .from("scoring_rules")
        .upsert(rows, {
          onConflict: "tournament_id,placement"
        });
    ...
    return NextResponse.json(
      { ok: false, error: "Unable to save point settings." },
      { status: 500 }
    );
  }
}
